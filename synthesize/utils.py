...
def selector(n, model, images, labels, size, m=5):
    ...
    dist_total = dist[indices]
    diffi = torch.stack([dist_total[i]/dist_total[0] for i in range(dist_total.shape[0])])
    
    return images[indices].detach(), diffi
